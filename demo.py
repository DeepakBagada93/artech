...
class UploadForm(FlaskForm):
    email_addresses = TextAreaField('Enter Email Addresses (comma-separated)', validators=[DataRequired()])
...
def send_email(subject, body, to_email):
    ...
    sender_email = 'deeepakbagada25@gmail.com'
    sender_name = 'Artechway'
...
def index():
    ...
    if form.validate_on_submit():
        email_addresses = [email.strip() for email in form.email_addresses.data.split(',')]
        ...
        # Send emails to each entered address
        for recipient_email in email_addresses:
            print(f"Sending email to: {recipient_email}")
            if validate_email(recipient_email):
                send_email(email_subject, full_email_body, recipient_email)
            else:
                print(f"Invalid email address: {recipient_email}")

        # Pass the email list to the template for display
        return render_template('index.html', form=form, email_list=email_list)

    return render_template('index.html', form=form, email_list=None)
